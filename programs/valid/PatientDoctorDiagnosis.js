...
db.connect(function(err) {
	if (err) console.log(err);
		...
		db.query('select * from Patient where  (Patient.DateOfBirth > 1950 AND Patient.DateOfBirth < 1970) AND  (Patient.PatientSerNum > 1 AND Patient.PatientSerNum < 2500) AND  (Patient.Sex like "m%")', function(err, rows, fields) {
			if (err) throw err;
...
function display(rows) {
	...
	for (var i_Diagnosis = 0; i_Diagnosis  < rows.length; i_Diagnosis) {
	 ...
	 console.log(Diagnosis);		table.push(generatePrettyRow(Diagnosis));
	}
	return table;
